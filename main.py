...
async def check_ban_command(ctx):
    ...
    # ✅ هنا أضفت رسالة الانتظار
    wait_message = {
        "en": f"{ctx.author.mention} ⏳ Processing, please wait...",
        "fr": f"{ctx.author.mention} ⏳ Traitement en cours, veuillez patienter..."
    }
    msg = await ctx.send(wait_message[lang])

    async with ctx.typing():
        try:
            ban_status = await check_ban(user_id)
        except Exception as e:
            await msg.edit(content=f"{ctx.author.mention} ⚠️ Error:\n{str(e)}")
            return
        ...
        if ban_status is None:
            ...
            await msg.edit(content=message[lang])
            return
